...
def transform_buffer(buff):
    s_batch, goal_batch, speed_batch, position_batch,adj_batch, a_batch, r_batch, d_batch, l_batch, \
    v_batch, last_q_batch = [], [], [], [], [], [], [], [], [], [], []
    s_temp, goal_temp, speed_temp, position_temp,adj_temp = [], [], [], [], []
    ...
    for e in buff:
        for state in e[0]:
            ...
            position_temp.append(state[3])
            adj_temp.append(state[4])
        s_batch.append(s_temp)
        goal_batch.append(goal_temp)
        speed_batch.append(speed_temp)
        position_batch.append(position_temp)
        adj_batch.append(adj_temp)
        s_temp = []
        goal_temp = []
        speed_temp = []
        position_temp = []
    ...
    last_q_batch = np.asarray(last_q_batch)

    return s_batch, goal_batch, speed_batch, position_batch, adj_batch, a_batch, r_batch, d_batch, l_batch, v_batch, last_q_batch


def generate_action(env, state_list, policy, action_bound):
    if env.index == 0:
        s_list, goal_list, speed_list, position_list, adj_list = [], [], [], [], []
        for i in state_list:
            s_list.append(i[0])
        ...
        adj_list = Variable(torch.from_numpy(adj_list)).float().cuda()
        ...
        a_wheel = []
        for action in a:
            v_left = action[0] - (action[1] * 0.16) / 2
            v_right = action[0] + (action[1] * 0.16) / 2
            a_wheel.append([v_left / 0.0975,v_right / 0.0975])
        a_wheel = np.array(a_wheel)
        # v_left = a[0][0] - (a[0][1]*0.16)/2
        # v_right = a[0][0] + (a[0][1] * 0.16) / 2
        # a_wheel = [[v_left / 0.0975,v_right / 0.0975]]
        # print(a,a_wheel)
        scaled_action = np.clip(a_wheel, a_min=action_bound[0], a_max=action_bound[1])
        # scaled_action = a
        ...
        scaled_action = None
        logprob = None
        all_attend_probs = None

    return v, a, logprob, scaled_action,all_attend_probs
...
def generate_action_no_sampling(env, state_list, policy, action_bound):
    if env.index == 0:
        ...
        _, _, _, mean,_ = policy(s_list, goal_list, speed_list)
...
def generate_train_data(rewards, gamma, values, last_value, dones, lam):
    ...
    for t in range(num_step - 1, -1, -1):
        ...
        targets[t, :] = gae + values[t, :]   # gae is a method to evaluate the advantage

    advs = targets - values[:-1, :]
    return targets, advs   # targets means evaluation of action value function



def ppo_update_stage1(policy, optimizer, batch_size, memory, epoch,
               coeff_entropy=0.02, clip_value=0.2,
               num_step=2048, num_env=12, frames=1, obs_size=24, act_size=4, global_update=0):
    obss, goals, speeds, positions,adjs, actions, logprobs, targets, values, rewards, advs,_,_ = memory

    advs = (advs - advs.mean()) / advs.std()

    # obss = obss.reshape((num_step*num_env, frames, obs_size))   # [num_step, num_env, frames, obs_size]
    # goals = goals.reshape((num_step*num_env, 2))
    # speeds = speeds.reshape((num_step*num_env, 2))
    # positions = positions.reshape((num_step * num_env, 3))
    ...
    for update in range(epoch):
        ...
        for i, index in enumerate(sampler):
            ...
            sample_adjs = Variable(torch.from_numpy(adjs[index])).float().cuda()
            ...
            new_value, new_logprob, dist_entropy = policy.evaluate_actions(sampled_obs, sampled_goals, sampled_speeds, sample_positions,sample_adjs, sampled_actions)
            ...
            #Does mean() effect the result?  No!
            policy_loss = -torch.min(surrogate1, surrogate2).mean()

            sampled_targets = sampled_targets.view(-1, 1)
            value_loss = F.mse_loss(new_value, sampled_targets)
            loss = policy_loss + 20 * value_loss - coeff_entropy * dist_entropy   #why entroy decereases over time?
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
            ...
            # writer.add_scalar('policy_loss',info_p_loss,global_step=global_update)
            # writer.add_scalar('value_loss', info_v_loss, global_step=global_update)
            # writer.add_scalar('entropy', info_entropy, global_step=global_update)


    now = datetime.datetime.now()
    print ('update at ', now.strftime('%X'))
...
def ppo_update_stage2(policy, optimizer, batch_size, memory, filter_index, epoch,
    ...
    #     original_weight = copy.deepcopy(policy).state_dict()['act_fea_cv1.weight'][0,0]
    # print(policy.state_dict()['act_fea_cv1.weight'][0,0]==original_weight)
    ...
    # obss = np.delete(obss, filter_index, 0)     # If I delete above 7 lines, which message should be filter is also changed
    ...
    #     # obss[filter[0],filter[1]] = None
    #     # goals[filter[0],filter[1]] = None
    #     # speeds[filter[0],filter[1]] = None
    #     # positions[filter[0],filter[1]] = None
    #     # actions[filter[0],filter[1]] = None
    #     # logprobs[filter[0],filter[1]] = None
    #     advs[filter[0],filter[1]] = None
    #     targets[filter[0],filter[1]] = None
    ...
    for update in range(epoch):
        sampler = BatchSampler(SubsetRandomSampler(list(range(advs.shape[0]))), batch_size=batch_size,
                               drop_last=False)
        for i, index in enumerate(sampler):
            #find the new index, and delete it (beacause of random sample
            #print 'training......',i
            ...
            new_value, new_logprob, dist_entropy = policy.evaluate_actions(sampled_obs, sampled_goals, sampled_speeds,
                                                                           sample_positions,sample_adjs, sampled_actions,this_filter_index)
            for this_filter in this_filter_index:
                sampled_targets[this_filter[0],this_filter[1]] *= 0
                sampled_advs[this_filter[0], this_filter[1]] *= 0
                new_value[this_filter[0], this_filter[1]] *= 0
                new_logprob[this_filter[0], this_filter[1]] *= 0
            ...
            surrogate1 = ratio * sampled_advs
            surrogate2 = torch.clamp(ratio, 1 - clip_value, 1 + clip_value) * sampled_advs
            # sum = torch.Tensor(1).to('cuda')
            # num = 0.0

            sampled_targets = sampled_targets.view(-1, 1)
            # for filter in filter_index:
            #     sampled_targets = sampled_targets[torch.arange(sampled_targets.size(0)) != filter[3]]

            # while True:
            #     have_nan = False
            #     for i in range(surrogate1.shape[0]):
            #         if torch.isnan(surrogate1[i]):
            #             have_nan = True
            #             surrogate1 = surrogate1[torch.arange(surrogate1.size(0)) != i]
            #             surrogate2 = surrogate2[torch.arange(surrogate2.size(0)) != i]
            #             sampled_targets = sampled_targets[torch.arange(sampled_targets.size(0)) != i]
            #             new_value = new_value[torch.arange(new_value.size(0)) != i]
            #             break
            #     if have_nan is False:
            #         break
            ...
            loss *= float(sampled_targets.shape[0])/(sampled_targets.shape[0]-len(this_filter_index))
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
            info_p_loss, info_v_loss, info_entropy = float(policy_loss.detach().cpu().numpy()), \
